...
mean_sample_vector <-  mean(example_vector)
round(mean_sample_vector,2)
```
...
#select gives back a data frame, while pull gives you a vector
```
...
nerd_red <- nerd_red %>% 
  ...
  select(age, age_character, married, everything())
```
